...
LlamaIndex supports Jmemory as a [memory store](https://llamahub.ai/l/memory/llama-index-memory-jmemory). In this guide, we'll show you how to use it.

<Note type="info">
  🎉 Exciting news! [**JmemoryMemory**](https://docs.llamaindex.ai/en/stable/examples/memory/JmemoryMemory/) now supports **ReAct** and **FunctionCalling** agents.
</Note>
...
```bash
pip install llama-index-core llama-index-memory-jmemory
```

### Setup with Mem0 Platform

Set your Jmemory Platform API key as an environment variable. You can replace `<your-jmemory-api-key>` with your actual API key:

<Note type="info">
  Ensure your Jmemory API key is securely configured in your environment.
</Note>

```python
os.environ["JMEMORY_API_KEY"] = "<your-jmemory-api-key>"
```

Import the necessary modules and create a JmemoryMemory instance:
```python
from llama_index.memory.mem0 import JmemoryMemory

context = {"user_id": "user_1"}
memory_from_client = JmemoryMemory.from_client(
    context=context,
...
Context is used to identify the user, agent or the conversation in the Mem0. It is required to be passed in the at least one of the fields in the `JmemoryMemory` constructor. It can be any of the following:
...
Create a JmemoryMemory instance:

```python
memory_from_config = JmemoryMemory.from_config(
    context=context,
    config=config,
    search_msg_limit=4,  # optional, default is 5
)
