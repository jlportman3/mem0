...
Jmemory seamlessly integrates with [Pipecat](https://pipecat.ai), providing long-term memory capabilities for conversational AI agents. This integration allows your Pipecat-powered applications to remember past conversations and provide personalized responses based on user history.
...
```bash
pip install "pipecat-ai[jmemory]"
```
...
You can obtain a Jmemory API key by signing up at [jmemory.ai](https://jmemory.ai).

## Configuration

Mem0 integration is provided through the `JmemoryMemoryService` class in Pipecat. Here's how to configure it:

```python
from pipecat.services.mem0 import JmemoryMemoryService

memory = JmemoryMemoryService(
...
The `JmemoryMemoryService` should be positioned between your context aggregator and LLM service in the Pipecat pipeline:
